...
__constant__ float d_DCTv8matrix[N * N];
__constant__ float d_DCTv8matrixT[N * N];
__constant__ float d_quantizationTable[N * N];
...
__global__ void dct(unsigned char *channel, float *dct, float *idct, int numRowsint, int numCols)

{
        ...
        for (i = 0; i < N; ++i)
        {
                ...
                ATX[threadIdx.y * blockDim.x + threadIdx.x] += d_DCTv8matrixT[threadIdx.y * blockDim.x + i] * channel[(blockIdx.y * blockDim.y + i) * numCols + blockIdx.x * blockDim.x + threadIdx.x];
        }      
        __syncthreads(); 
        ...
        for (i = 0; i < N; ++i)
        {
                 ...
                 dct[thread_1D_pos] += ATX[threadIdx.y * blockDim.x + i] * d_DCTv8matrix [i * blockDim.x + threadIdx.x];
        }

        __syncthreads();

       
        dct[thread_1D_pos] *= d_quantizationTable[threadIdx.y * blockDim.x + threadIdx.x];
        ...
        for (i = 0; i < N; ++i)
        {
                ATX[threadIdx.y * blockDim.x + threadIdx.x] += d_DCTv8matrix[threadIdx.y * blockDim.x + i] * dct[(blockIdx.y * blockDim.y + i) * numCols + blockIdx.x * blockDim.x + threadIdx.x];
        }       
        ...
        for (i = 0; i < N; ++i)
        {
                 idct[thread_1D_pos] += ATX[threadIdx.y * blockDim.x + i] * d_DCTv8matrixT[i * blockDim.x + threadIdx.x];
        }
...
int main(int argc, char *argv[])
{
        ...
        BMPData *image1 = NULL;
        FILE *fp;
        char *remark = "With Constant memory usage";
        int i = 0,j = 0;
        int size = 0;
        ...
        gpuErrchk(cudaMemcpyToSymbol(d_DCTv8matrix, &DCTv8matrix, N * N * sizeof(float)));
        gpuErrchk(cudaMemcpyToSymbol(d_DCTv8matrixT, &DCTv8matrixT, N * N * sizeof(float)));
        gpuErrchk(cudaMemcpyToSymbol(d_quantizationTable, &quantizationTable, N * N * sizeof(float)));
        ...
        dct<<<  grid, block >>>(d_red, d_red_dct, d_red_idct, image1->infoHeader.height,image1->infoHeader.width);
        dct<<<  grid, block >>>(d_green, d_green_dct, d_green_idct, image1->infoHeader.height,image1->infoHeader.width);
        dct<<<  grid, block >>>(d_blue, d_blue_dct, d_blue_idct, image1->infoHeader.height,image1->infoHeader.width);
