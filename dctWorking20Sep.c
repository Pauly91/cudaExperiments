...
__global__ void dct(unsigned char *channel, float *dct, float *idct, float *quantizationTable, int numRowsint, int numCols, float *DCTv8matrix, float *DCTv8matrixT )
{
        ...
        //for (i = 0; i < N*N; ++i)
        //{
        ATX[threadIdx.y * blockDim.x + threadIdx.x] = 0;
        sQuantizationTable[threadIdx.y * blockDim.x + threadIdx.x] = quantizationTable[threadIdx.y * blockDim.x + threadIdx.x];
        sDCTv8matrix[threadIdx.y * blockDim.x + threadIdx.x] = DCTv8matrix[threadIdx.y * blockDim.x + threadIdx.x];
        sDCTv8matrixT[threadIdx.y * blockDim.x + threadIdx.x] = DCTv8matrixT[threadIdx.y * blockDim.x + threadIdx.x];
        ...
        for (i = 0; i < N; ++i) // looping over shared mem is a prob - bank conflict ?
        {
                ...
                ATX[threadIdx.y * blockDim.x + threadIdx.x] += sDCTv8matrix[i * blockDim.x + threadIdx.y] * channelData[i][threadIdx.x];
        ...
        for (i = 0; i < N; ++i) // looping over shared mem is a prob - bank conflict ?
        {
                 ...
                 //dct[thread_1D_pos] += ATX[threadIdx.y * blockDim.x + i] * sDCTv8matrixT[threadIdx.y * blockDim.x + i];
        }
        ...
        for (i = 0; i < N; ++i) // looping over shared mem is a prob - bank conflict ?
        {
                ATX[threadIdx.y * blockDim.x + threadIdx.x] += sDCTv8matrixT[i * blockDim.x + threadIdx.y] * dct[(blockIdx.y * blockDim.y + i) * numCols + blockIdx.x * blockDim.x + threadIdx.x];
        }       
        ...
        for (i = 0; i < N; ++i) // looping over shared mem is a prob - bank conflict ?
        {
                 idct[thread_1D_pos] += ATX[threadIdx.y * blockDim.x + i] * sDCTv8matrixT[i * blockDim.x + threadIdx.x];
